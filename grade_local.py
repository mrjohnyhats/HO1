...
class T4A(cse40.question.Question):
    def score_question(self, submission, synthetic_data):
        result = submission.histogram_titer(synthetic_data)
        if (self.check_not_implemented(result)):
            return

        if (isinstance(result, numpy.ndarray)):
            if (result.shape != (1, 1)):
                self.fail("Answer does not contain a histogram.")
                return
            result = result[0][0]

        if (not isinstance(result, matplotlib.axes._subplots.SubplotBase)):
            self.fail("Answer must be a histogram.")
            return

        self.full_credit()

class T4B(cse40.question.Question):
    def score_question(self, submission, synthetic_data):
        result = submission.scatter_day_titer(synthetic_data)
        if (self.check_not_implemented(result)):
            return

        if (not isinstance(result, matplotlib.axes._subplots.SubplotBase)):
            self.fail("Answer must be a scatter plot.")
            return
...
def grade(path):
    ...
    questions = [
        ...
        T4A("Task 4.A (histogram_titer)", 1),
        T4B("Task 4.B (scatter_day_titer)", 1),
